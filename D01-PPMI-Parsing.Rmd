...
dumpDir <- "~/Shared/Data-Science/PPMI/Data/Data-2019-02-06"
...
dupPatEv <- updrs3.pre[which(duplicated(updrs3.pre[,1:2])), 1:2]
if(nrow(dupPatEv)==0){
...
dupPatEv <- updrs3.post[which(duplicated(updrs3.post[,1:2])), 1:2]
if(nrow(dupPatEv)!=0){
...
dupPatEv <- updrs5.pre[which(duplicated(updrs5.pre[,1:2])), 1:2]
if(nrow(dupPatEv)==0){
    stop("No duplicated rows ==> review this section!")
}
```
...
dupPatEv <- updrs5.post[which(duplicated(updrs5.post[,1:2])), 1:2]
if(nrow(dupPatEv)!=0){
    stop("Duplicated rows ==> review this section!")
}
...
dupPatEv <- dyskDoc.pre[which(duplicated(dyskDoc.pre[,1:2])), 1:2]
if(nrow(dupPatEv)==0){
    stop("No duplicated rows ==> review this section!")
}
```
...
dupPatEv <- dyskDoc.post[which(duplicated(dyskDoc.post[,1:2])), 1:2]
if(nrow(dupPatEv)!=0){
    stop("Duplicated rows ==> review this section!")
}
...
#         invert=T, value=T
# )){
#     cranNerves[,cn] <- as.logical(cranNerves[,cn])
# }
takenFiles <- unique(c(takenFiles, f))
```
...
rownames(pdMed) <- paste(pdMed$PATNO, pdMed$EVENT_ID, sep=".")
# cn <- "PDMEDYN"
...
factCol <- setdiff(unique(tmp$TESTNAME[which(is.na(tmp$UNITS))]), "SCORE")
...
for(cn in numCol){
	...
	convVal[which(convVal %in% c("Undetermined", "N/A"))] <- NA
...
```{r, echo=FALSE, message=FALSE}
setdiff(availableFiles, takenFiles)
```
